...
NoximNoP_data NoximHMRouter::getCurrentNoPData() const
{
    NoximNoP_data NoP_data;

    for (int j = 0; j < DIRECTIONS_HM; j++) {
...
int NoximHMRouter::NoPScore(const NoximNoP_data & nop_data,
...
int NoximHMRouter::selectionNoP(const vector < int >&directions,
...
vector < int >NoximHMRouter::routingWestFirst(const NoximCoord & current,
					    const NoximCoord & destination)
{
    vector < int >directions;
...
vector < int >NoximHMRouter::routingNorthLast(const NoximCoord & current,
					    const NoximCoord & destination)
{
    vector < int >directions;
...
vector < int >NoximHMRouter::routingNegativeFirst(const NoximCoord & current,
						const NoximCoord &
						destination)
{
    vector < int >directions;
...
vector < int >NoximHMRouter::routingOddEven(const NoximCoord & current,
...
vector < int >NoximHMRouter::routingDyAD(const NoximCoord & current,
				       const NoximCoord & source,
				       const NoximCoord & destination)
{
    vector < int >directions;
...
vector < int >NoximHMRouter::routingFullyAdaptive(const NoximCoord & current,
						const NoximCoord &
						destination)
{
    vector < int >directions;
...
vector < int >NoximHMRouter::routingTableBased(const int dir_in,
...
void NoximHMRouter::configure(const int _id,
